...
if __name__ == "__main__":
    ...
    pitchspd, yawspd, rollspd = 0, 0, 1
    pitchspd_acc, yawspd_acc, rollspd_acc = 0, 1, 2
    
    start = time.time()
    for dpsr in [10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60]:
        np.random.seed(1)
        for i in range(1000):
            
            # Log
            curr = time.time()
            print('[dpsr = {} id = {} time : {:.1f}s]'.format(dpsr, i, curr - start))
            ...
            # Generate.
            savename = '/media/wavenz/新加卷/graph/dpsr{}/{}.npy'.format(dpsr, i)
            ...
            x = G.generate(pitch, yaw, roll, dpsr * pitchspd, dpsr * yawspd, dpsr * rollspd, exposure=100, starsize=1.2, noise=3, savename=savename)
   
            # # Save
            # img = Image.fromarray(img)
            # img = img.convert('L')
            # img.save('E:/graph/dpsr{}/{}.png'.format(dpsr, i))

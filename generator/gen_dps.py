...
class StarGenerator(object):
    ...
    def generate(self, pitch, yaw, roll, pitchspd, yawspd, rollspd, exposure = 100, 
                 starsize = 1.3, winvisible = False, winradius = 50, noise = 3, savename = './null.npy'):
        '''Generate star image.'''
...
if __name__ == "__main__":
    ...
    pitchspd, yawspd, rollspd = 1/2, 1.732/2, 0
    pitchspd_acc, yawspd_acc, rollspd_acc = 0, 1, 2
    
    start = time.time()

    for dps in range(10, 13):
        np.random.seed(1)
        for i in range(1000):
            
            # Log
            curr = time.time()
            print('[dps = {} id = {} time : {:.1f}s]'.format(dps, i, curr - start))
            ...
            savename = '/media/wavenz/新加卷/graph/dps{}/{}.npy'.format(dps, i)
            x = G.generate(pitch, yaw, roll, dps * pitchspd, dps * yawspd, dps * rollspd, exposure=100, starsize=1.2, noise=3, savename=savename)
            # x = x.T
            # plt.figure()
            # plt.imshow(retImg, cmap='gray', vmin=0, vmax=255)
            # plt.show()
            # np.save('/media/wavenz/新加卷/graph/dps{}/{}.npy'.format(dps, i), x)
            # # Save
            # img = Image.fromarray(img)
            # img = img.convert('L')
            # img.save('E:/graph/dps{}/{}.png'.format(dps, i))

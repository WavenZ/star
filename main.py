...
if __name__ == "__main__":
    ...
    for image in images:
        ...
        # print(src.shape)
        # src = (np.random.randn(2048, 2048) * 2 + 20).astype('uint8')
        blur = cv2.blur(src, (3, 3)).astype('uint8')
        theta = ae.Direction_estimate(blur)
        # theta = [1024, 1024]
        retImg = ex.extract(src, theta)
        plt.figure()
        plt.imshow(np.hstack((src, retImg)), cmap='gray', vmin=0, vmax=255)
        plt.show()
